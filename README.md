...
> **Note:** Steps 04–05 (FIC token chain) use `client_credentials` flows against the Entra token endpoint, which blocks browser CORS by design. The app tries direct calls first and automatically falls back to generating **curl/PowerShell** commands if CORS blocks the request — just run the command in a terminal and paste the JSON response back.
